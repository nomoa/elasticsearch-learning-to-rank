...
public class FeatureMatrixCollectorTests extends LuceneTestCase {
    ...
    public void testEmptyInitial() throws IOException {
        ...
        FeatureMatrixCollector collector = new FeatureMatrixCollector((c) -> Collections.emptyList(),
    ...
    public void testRandom() throws IOException {
        ...
        List<Weight> weights = features.stream().map((q) -> {
            try {
                return searcher.createNormalizedWeight(q, true);
            } catch (IOException e) {
                throw new RuntimeException(e);
            }
        }).collect(Collectors.toList());
        CheckedFunction<LeafReaderContext, List<Scorer>, IOException> scorers = (ctx) -> weights.stream().map((w) -> {
            try {
                return w.scorer(ctx);
            } catch (IOException e) {
                throw new RuntimeException(e);
        ...
        FeatureMatrixCollector collector = new FeatureMatrixCollector(scorers, searcher.getIndexReader(), docs.scoreDocs, windowSize);
