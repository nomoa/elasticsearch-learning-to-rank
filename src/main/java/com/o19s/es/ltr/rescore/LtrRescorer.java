...
public class LtrRescorer implements Rescorer {
    ...
    public TopDocs rescore(TopDocs topDocs, IndexSearcher searcher, RescoreContext rescoreContext) throws IOException {
        ...
        if (ctx.rankerQuery.getRanker() instanceof BulkLtrRanker && ctx.batchSize != 1) {
    ...
    TopDocs bulkRescore(TopDocs topDocs, IndexSearcher searcher, LtrRescoreContext ctx) throws IOException {
        ...
        Weight weight = searcher.createWeight(searcher.rewrite(ctx.rankerQuery), true, 1F);
        BulkLtrRanker ranker = (BulkLtrRanker) ctx.rankerQuery.getRanker();
        assert weight instanceof RankerQuery.RankerWeight;

        int wSize = Math.min(ctx.getWindowSize(), docs.length);
        FeatureMatrixCollector collector = new FeatureMatrixCollector((RankerQuery.RankerWeight) weight,
            searcher.getIndexReader(), docs, wSize);
    ...
    public TopDocs classicRescore(TopDocs topDocs, IndexSearcher searcher, LtrRescoreContext ctx) throws IOException {
        QueryRescorer rescorer = new QueryRescorer(ctx.rankerQuery) {
            @Override
    ...
    public Explanation explain(int topLevelDocId, IndexSearcher searcher, RescoreContext rescoreContext,
        ...
        Explanation rescoreQueryExplanation = searcher.explain(((LtrRescoreContext) rescoreContext).getRankerQuery(), topLevelDocId);
    ...
    public void extractTerms(IndexSearcher searcher, RescoreContext rescoreContext, Set<Term> termsSet) throws IOException {
        searcher.createWeight(searcher.rewrite(((LtrRescoreContext) rescoreContext).rankerQuery), true, 1F).extractTerms(termsSet);
    }

    public static class LtrRescoreContext extends RescoreContext {
        public static final int DEFAULT_WINDOW_SIZE = 10;
        private RankerQuery rankerQuery;
        ...
        public RankerQuery getRankerQuery() {
            return rankerQuery;
        }

        public LtrRescoreContext setRankerQuery(RankerQuery rankerQuery) {
            this.rankerQuery = rankerQuery;
            return this;
        }
