...
public class LtrRescoreBuilder extends RescorerBuilder<LtrRescoreBuilder> {
    ...
    protected RescoreContext innerBuildContext(int windowSize, QueryShardContext context) throws IOException {
        ...
        ctx.setRankerQuery((RankerQuery) this.query.toQuery(context));
    ...
    public LtrRescoreBuilder setQuery(QueryBuilder query) {
        if (!(query instanceof LtrQueryBuilder || query instanceof StoredLtrQueryBuilder)) {
            throw new IllegalArgumentException(RESCORE_QUERY.getPreferredName() + " must be a [" + StoredLtrQueryBuilder.NAME + "] " +
                    "or a [" + LtrQueryBuilder.NAME + "]");
        }
        this.query = query;
        return this;
    }
